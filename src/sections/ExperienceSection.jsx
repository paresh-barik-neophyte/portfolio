...
const ExperienceSection = () => {
  ...
  return (
    <section
      id="experience"
      ref={sectionRef}
      className="py-24 bg-gradient-to-br from-slate-50 via-white to-blue-50 dark:from-slate-900 dark:via-slate-800 dark:to-blue-900/20"
    >
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <div ref={titleRef} className="text-center mb-20">
          ...
          <p className="text-xl text-muted-foreground max-w-3xl mx-auto leading-relaxed">
            My career progression through different roles, responsibilities, and
            achievements in the frontend development landscape, building
            impactful solutions.
          </p>
        </div>
          ...
          {experience.map((job, index) => (
            <div key={job.id} className="timeline-item">
              <div className="bg-white dark:bg-slate-800 rounded-3xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-500 border border-slate-200 dark:border-slate-700 group">
                {/* Header Section */}
                <div
                  className={`bg-gradient-to-r ${gradients[index]} p-8 text-white relative overflow-hidden`}
                >
                        ...
                        <div key={achIndex} className="group/achievement">
                          <div className="bg-slate-50 dark:bg-slate-700 rounded-xl p-6 h-full border border-slate-200 dark:border-slate-600 hover:bg-slate-100 dark:hover:bg-slate-600 transition-colors duration-300">
                            <div className="flex items-start space-x-4">
                              <div
                                className={`w-8 h-8 bg-gradient-to-r ${gradients[index]} rounded-lg flex items-center justify-center flex-shrink-0 mt-1`}
                              >
                ...
                <div
                  className={`h-1 bg-gradient-to-r ${gradients[index]}`}
                ></div>
              </div>
            </div>
          ))}
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
              {[
                ...
                <div key={index} className="text-center group">
                  <div
                    className={`text-5xl md:text-6xl font-black bg-gradient-to-r ${stat.gradient} bg-clip-text text-transparent mb-2 group-hover:scale-110 transition-transform duration-300`}
                  >
                    {stat.number}
                  </div>
